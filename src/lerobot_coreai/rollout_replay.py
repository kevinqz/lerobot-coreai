...
from .rollout_evidence_schema import REQUIRED_CHECKS, canonical_json_sha256
...
def _wire_ok(raw) -> bool:
    ...
    for body in raw["request_bodies"]:
        ...
        if native and B > 1 and opts.get("batch_size") != B:
            return False
        if not (isinstance(opts.get("observation_sha256"), str)
                and isinstance(opts.get("observation_encoding"), str)):
            return False
    return True
...
def _chain_ok(raw) -> bool:
    ...
    if _per_sample_shape(final) != [B, seq, A]:
        return False
    ...
    for t in range(seq):
        ...
        for b in range(B):
            if native and B > 1:
                chunk = responses[pred]                # [B,H,A]
                expected = chunk[b][hidx]
            else:                                      # single / split: [H,A] per sample
