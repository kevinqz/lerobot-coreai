...
v1.3.5 hardens the runner protocol and proves the chain composes **without mocks**: a `NegotiatedRunnerProtocol` validates the runner's announced `protocol_version` against `minimum_runner_protocol` and sends the *negotiated* version (never a hardcoded constant); a capabilities/transport failure now **propagates** instead of silently falling back to legacy (legacy is opt-in via `allow_legacy_runner_protocol`); the `TypeError` compatibility fallback around `predict_action_chunk` is removed (companion requires `lerobot-coreai>=1.3.5`); action-contract parsing **fails closed** (invalid contract raises; `representation=single ⇒ horizon=1` is enforced). A hermetic B=1 end-to-end test runs a real local HTTP runner (`/v1/health`, `/v1/capabilities`, `/v1/predict`) through a real `CoreAIBridgePolicy.from_pretrained → CoreAIPolicy.from_pretrained → RunnerClient → negotiation → POST → strict validation → Tensor[1, A]` with **nothing patched**, asserting wire payload, no label leakage, no secret persistence, and fail-closed failure paths. Serializable `PolicyProcessorPipeline` + canonical publishable artifact + full `make_policy`/`make_pre_post_processors` composition remain v1.3.6. See [docs/official-lerobot-plugin.md](docs/official-lerobot-plugin.md).
